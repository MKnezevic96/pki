     ...
     public OCSPResp generateOCSPResponse(Certificate subject, Certificate issuer,
        ...
        try {
            ...
            BasicOCSPRespBuilder basicBuilder = new BasicOCSPRespBuilder(
                    SubjectPublicKeyInfo.getInstance(issuerCert.getPublicKey().getEncoded()),
                    digCalcProv.get(CertificateID.HASH_SHA1));

            CertificateID certId = new CertificateID(digCalcProv.get(CertificateID.HASH_SHA1),
                    issuerCertHolder, subjectCert.getSerialNumber());
            ...
            BasicOCSPResp resp = basicBuilder.build(
                    new JcaContentSignerBuilder("SHA256withECDSA").build(issuerPrivateKey), chain, new Date());
